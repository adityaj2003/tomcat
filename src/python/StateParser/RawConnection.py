...
# COMMENT THIS OUT WHEN NOT RUNNING FROM CONFIG.JSON FILE SETTINGS (OUTSIDE OF DOCKER-COMPOSE)
...
broker = {
    ...
    'host': os.getenv('MQTT_BROKER_HOSTNAME', data['host']),
...
class RawConnection(object):
    def __init__(self, clientid, host=broker['host'], port=broker['port'],keepalive=broker['keepalive'], bind_address=broker['bind_address']):
        ...
        self.__mqttc.on_disconnect = self.__on_connect
    ...
    def connect(self,start_loop=True):
        ...
        self.__mqttc.connect(self.__host, port=self.__port, keepalive=self.__keepalive, bind_address=self.__bind_address)
