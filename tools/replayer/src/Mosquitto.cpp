...
void MosquittoListener::on_message(const string& topic, const string& message) {
	...
	if(sub_type.compare("start") == 0){
		std::string timestamp = m["msg"]["timestamp"];			
		this->start_study = string_to_time_point(timestamp);			
...
chrono::time_point<chrono::system_clock> MosquittoListener::string_to_time_point(string timestamp){
	size_t pos = timestamp.find(".");
	std::string time = timestamp.substr(0, pos);
